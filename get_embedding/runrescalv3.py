...
class SideMapping():
    ...
    def merge_all(self,ec,em,epa,epe,ee,mc,e1e2,specific="gene"):
        entity = [k for d in [ec,em,epa,epe,ee,mc,e1e2] for k in d.keys()]
    ...
    def entity2id(self):
        ...
        diseases = self.merge_all(self.dc,self.dm,self.dpa,self.dpe,self.dd,self.mc,self.dg)
        genes = self.merge_all(self.gc,self.gm,self.gpa,self.gpe,self.gg,self.mc,self.gd)
        # map to id
...
class BuildW():
    def __init__(self,chemical2id,mutation2id,pathway2id,phenotype2id,microrna2id,disease2indexpath,
                gene2indexpath,gene2id,disease2id,dc,dd,dm,dpa,dpe,gc,gg,gm,gpa,gpe,mc,dg,gd):
...
def main():
    ...
    #构建边缘矩阵W
    BW = BuildW(chemical2id,mutation2id,pathway2id,phenotype2id,microrna2id,disease2indexpath,
                gene2indexpath,gene2id,disease2id,SM.dc,SM.dd,SM.dm,SM.dpa,SM.dpe,
                 SM.gc,SM.gg,SM.gm,SM.gpa,SM.gpe,SM.mc,SM.dg,SM.gd)
