...
class SideMapping():
    ...
    def pipline(self,importantdiseases,importantgenes,filter_flag):
        if filter_flag:
            ...
            # self.gd = {key:tuple(value) for key,value in self.gpe.items() if key in importantgenes and value.split(":")[-1] in importantdiseases}
            # self.dg = {key:tuple(value) for key,value in self.gpe.items() if value in importantgenes and key.split(":")[-1] in importantdiseases}
        else:
    ...
    def merge_all(self,dc,dm,dpa,dpe,dd,gd,specific="gene"):
        if specific=="gene":
    ...
    def entity2id(self):
        ...
        diseases = self.merge_all(self.dc,self.dm,self.dpa,self.dpe,self.dd,self.gd,"disease")
        genes = self.merge_all(self.gc,self.gm,self.gpa,self.gpe,self.gg,self.gd,"gene")
...
class BuildW():
    def __init__(self,chemical2id,mutation2id,pathway2id,phenotype2id,
                 gene2id,disease2id,dc,dd,dm,dpa,dpe,gc,gg,gm,gpa,gpe,gd,dg):
...
def main():
    ...
    parser.add_argument("--fit_method",help="fit_method")
    ...
    BW = BuildW(chemical2id,mutation2id,pathway2id,phenotype2id,
                 ...
                 SM.gc,SM.gg,SM.gm,SM.gpa,SM.gpe,SM.gd,SM.dg)
